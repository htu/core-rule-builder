...
def get_db_cfg (db_name:str='library',container_name:str=None):
    """
    ...
    database client, and container client (if container_name is provided and the container exists).
    ...
    db_cfg = { "url": url, "key": key,  "db_client": cln,
               "db_name": db_name, "db_conn": None,
               "ct_name": container_name, "ct_conn": None}
    ...
    if container_name is not None:
        try:
            v_stp = 1.31
            cont = db_conn.get_container_client(container_name)
            v_msg = "Found container - " + container_name
            v_msg += " in Cosmos DB - " + db_name
            echo_msg(v_prg, v_stp, v_msg,4)
            db_cfg["ct_conn"] = cont 
        except exceptions.CosmosResourceNotFoundError:
            v_stp = 1.32
            v_msg = "Could not find " + container_name + " in DB " + db_name 
...
if __name__ == "__main__":
    ...
    db_cfg = get_db_cfg(db_name='library', container_name='core_rules_dev')
    assert 'url' in db_cfg.keys()
    assert 'key' in db_cfg.keys()
    assert 'db_name' in db_cfg.keys()
    assert 'db_client' in db_cfg.keys()
